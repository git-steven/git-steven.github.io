# icon-lexicon.v5.md
Semantic emoji lexicon for architecture documentation, diagrams, and technical writing.

Icons function as **semantic anchors** that help readers scan technical documents quickly.

Designed for:

- Markdown documentation
- Architecture diagrams
- draw.io / diagrams.net
- Mermaid diagrams
- Jupyter notebooks
- Technical blog posts

---

## All Icons

🏛️ 🧱 🪨 🗿 🧬 📐 📏 🧭 🧮 🏗️ 🧰 ⚙️ 🧲 🏢 🏦 🏰 🏚️ 🏠 🏡 🏤 🚧 🚪 🪜 ♻️ 🧩 🗺️ 🌐 📍 🏷️ 🛣️ 🛤️ 🚦 ➡️ 🔎 🔭 🧵 🌉 📚 📖 📙 📗 📘 🗒️ 📝 📄 📑 🗃️ 🗄️ 🗂️ 🧠 🎓 🔍 📌 ✅ ⚠️ 📜 📃 🧾 🔌 🔖 ☑️ 🧪 📦 ➕ 🔗 🧷 👥 👤 🫂 🕴️ 🧍 🚶 👫 🧑 👨 👩 🧑‍💻 👷 🧑‍🔬 🧑‍🏭 🧑‍💼 🧑‍🎓 🧑‍🔧 🧑‍✈️ 🧑‍🚀 🧑‍⚕️ 🧑‍🏫 🕵️ 🌀 🌊 🔄 🎛️ 📣 📨 🛰️ 🟢 🟡 🔴 ☯ ⏱️ ⏳ 🕒 🕰 🚰 📈 📊 📉 📋 🧟 🪦 🧊 💻 🛠️ 🔧 🔩 ⛏️ 🪛 🪚 🔨 📡 🔋 🪫 💡 🔦 ⚖️ 🎯 🛡️ 🔐 🔒 🪪 🗝️ 🐞 🧨 🚨 🔬 ✔️ 🚀 🏁 🖥️ ☁️ 💧 🫧 💦 🔥 ⚡ 💥 🌱 🌿 🌳 ☀️ 🌙 ✨ 🌥️ 🌬️ 💨 🪁 🌪️ ⛈️ 🌧️ 🌩️ ☔ 🌤️ 🌦️ 🌡️ 🌫️ ☄️ 🌍 🌎 🌏 🪐 🌌 🛸 👽 🌠 🌕 🌑 🌒 🌓 🌔 🌖 🌗 🌘 💫 🏔️ ⛰️ 🗻 🏕️ 🏘️ 🏞️ 🏜️ 🏝️ 🏖️ 🌋 ⬅️ ⬆️ ⬇️ ↗️ ↘️ ↙️ ↖️ ↔️ ↕️ ⤴️ ⤵️ ↩️ ↪️ 🔃 🔁 🔂 ⭐ 🌟 ☢️ ☣️ ★ ☆ ✦ ✧ ✴ ✪ ⍟ ✫ ✯ ✰ ❂ ✹ ✸ ⋆ 👹 ⚓ ⏲ 🤷‍♂️ 🖼️ ⛓️ ⛓️‍💥 🔢 🔤 😎 🤖 🦾 🦿 🤝 🧫 🦠 ⚗️ ⚛️ ° 🧑‍🍳

---

## Table of Contents

1. [Architecture & Structure](#-architecture--structure)
2. [Context & System Landscape](#️-context--system-landscape)
3. [Documentation & Knowledge](#-documentation--knowledge)
4. [Contracts & Interfaces](#-contracts--interfaces)
5. [Components & Modules](#-components--modules)
6. [Flow & Lifecycle](#-flow--lifecycle)
7. [Lean Flow](#-lean-flow)
8. [Backlog & Work Management](#-backlog--work-management)
9. [People & Roles](#-people--roles)
10. [Engineering](#️-engineering)
11. [Science & Lab](#-science--lab)
12. [Space & Cosmic](#-space--cosmic)
13. [Analysis](#-analysis)
14. [Security](#️-security)
15. [Testing & Quality](#-testing--quality)
16. [Delivery & Operations](#-delivery--operations)
17. [System Dynamics](#-system-dynamics)
18. [Weather](#️-weather)
19. [External Environment](#-external-environment)
20. [Arrows & Direction](#️-arrows--direction)
21. [Stars & Decorative](#-stars--decorative)
22. [Hazard Symbols](#️-hazard-symbols)
23. [Diagram Grammar](#diagram-grammar)
24. [Canonical Icons](#canonical-icons)

---

# 🏛️ Architecture & Structure

| Category | Icons |
|---|---|
| **Foundations** | 🧱 brick · 🪨 rock · 🗿 moai · 🧬 dna · ⚓ anchor |
| **Design / Blueprint** | 📐 triangle ruler · 📏 ruler · 🧭 compass · 🧮 abacus |
| **Construction** | 🏗️ crane · 🧰 toolbox · ⚙️ gear · 🧲 magnet |
| **Institutions** | 🏛️ classical building · 🏢 office · 🏦 bank · 🏰 castle · 🏚️ derelict house |
| **Residential** | 🏠 house · 🏡 house w/garden · 🏘️ houses · 🏤 european post office |
| **Boundaries** | 🚧 construction · 🚪 door · 🪜 ladder |
| **Patterns** | ♻️ recycle · 🧩 puzzle piece |

---

# 🗺️ Context & System Landscape

| Category      | Icons                                                          |
|---------------|----------------------------------------------------------------|
| **Maps**      | 🗺️ world map · 🧭 compass · 🌐 globe                          |
| **Ownership** | 📍 pushpin · 🏷️ label                                         |
| **Routing**   | 🛣️ motorway · 🛤️ railway · 🚦 traffic light · ➡️ right arrow |
| **Discovery** | 🔎 magnifying glass right · 🔭 telescope · 🧵 thread           |
| **Crossings** | 🌉 bridge at night · 🚪 door · 🚧 construction                 |

---

# 📚 Documentation & Knowledge

| Category             | Icons                                                                   |
|----------------------|-------------------------------------------------------------------------|
| **References**       | 📚 books · 📖 open book · 📙 orange book · 📗 green book · 📘 blue book |
| **Working Docs**     | 🗒️ notepad · 📝 memo · 📄 page · 📑 bookmark tabs                      |
| **Knowledge Stores** | 🗃️ card file box · 🗄️ file cabinet · 🗂️ dividers · 🖼️ framed picture |
| **Learning**         | 🧠 brain · 🎓 graduation cap · 🔍 magnifying glass                      |
| **Guidance**         | 📌 pushpin · ✅ check mark · ⚠️ warning                                  |

---

# 📜 Contracts & Interfaces

| Category       | Icons                                        |
|:---------------|----------------------------------------------|
| **Contracts**  | 📜 scroll · 📃 page with curl · 🧾 receipt   |
| **APIs**       | 🔌 plug · ⚙️ gear                            |
| **Schemas**    | 🧬 dna · 📐 triangle ruler                   |
| **Versioning** | 🏷️ label · 🔖 bookmark · ♻️ recycle         |
| **Validation** | ✅ check mark · ☑️ ballot box · 🧪 test tube  |

---

# 🧩 Components & Modules
| Category                  | Icons                                                  |
|---------------------------|--------------------------------------------------------|
| **Components / Services** | 🧩 puzzle piece                                        |
| **Packages / Artifacts**  | 📦 package · 🗂️ dividers                              |
| **Composition**           | 🧩 puzzle piece · ➕ plus · 🏗️ crane                   |
| **Seams / Adapters**      | 🔌 plug · 🔗 link · 🧷 safety pin · ⛓️‍💥 broken chain |
| **Teams / Ownership**     | 👥 people · 🏷️ label · 🤝 handshake                   |

---
# 🌀 Flow & Lifecycle
| Category | Icons                                                                   |
|---|-------------------------------------------------------------------------|
| **Flow** | 🌀 cyclone · 🌊 wave · ➡️ right arrow · 🔄 cycle · 💦 afferent coupling |
| **Orchestration** | 🎛️ control knobs · 🧭 compass · 🧵 thread                              |
| **Async / Events** | 📣 megaphone · 📨 envelope · 🛰️ satellite                              |
| **Status** | 🟢 green · 🟡 yellow · 🔴 red · ✅ check · ⚠️ warning · ☯ yin yang       |
| **Time** | ⏱️ stopwatch · ⏲ timer · ⏳ hourglass · 🕒 clock · 🕰 mantel clock      |

---

# 🌊 Lean Flow

| Category                 | Icons                      |
|:-------------------------|:---------------------------|
| **Flow**                 | 🌊 wave                    |
| **Bottleneck**           | 🚰 potable water           |
| **Iteration**            | 🔄 cycle                   |
| **Throughput / Metrics** | 📈 chart up · 📊 bar chart |

---

# 🧟 Backlog & Work Management

| Category        | Icons                         |
|:----------------|:------------------------------|
| **Work Items**  | 📋 clipboard · 🧾 receipt     |
| **Waiting**     | ⏳ hourglass · 🕰 mantel clock |
| **Zombie Work** | 🧟 zombie                     |
| **Dead Work**   | 🪦 headstone                  |
| **Frozen Work** | 🧊 ice                        |
| **Pointless**   | 🤷‍♂️ shrug                     |

---

# 👥 People & Roles

| Category | Icons |
|---|---|
| **Individuals** | 👤 silhouette · 🧑 person · 👨 man · 👩 woman · 🕴️ levitating |
| **Groups** | 👥 silhouettes · 👫 couple · 🫂 hugs |
| **Posture** | 🧍 standing · 🚶 walking |
| **Tech Workers** | 🧑‍💻 technologist · 🧑‍🔧 mechanic · 🧑‍🏭 factory worker · 👷 construction worker |
| **Knowledge Workers** | 🧑‍💼 office worker · 🧑‍🎓 student · 🧑‍🏫 teacher · 🎓 graduation cap |
| **Science & Health** | 🧑‍🔬 scientist · 🧑‍⚕️ health worker · 🕵️ detective |
| **Exploration** | 🧑‍✈️ pilot · 🧑‍🚀 astronaut |

---

# ⚙️ Engineering

| Category              | Icons                                                                  |
|:----------------------|:-----------------------------------------------------------------------|
| **Coding**            | 🧑‍💻 technologist · 💻 laptop · 🖥️ desktop                           |
| **Core Tools**        | ⚙️ gear · 🧰 toolbox · 🛠️ hammer wrench · 🔧 wrench                  |
| **Hardware Tools**    | 🔩 nut and bolt · 🔨 hammer · ⛏️ pick · 🪛 screwdriver · 🪚 saw       |
| **Electrical**        | 🔋 battery · 🪫 low battery · 💡 light bulb · 🔦 flashlight · ⚡ high voltage |
| **Communications**    | 📡 satellite dish · 🛰️ satellite                                      |
| **Execution Threads** | 🧵 thread                                                              |
| **Robotics & AI**     | 🤖 robot · 🦾 mechanical arm · 🦿 mechanical leg · 🧠 brain · ✨ sparkles |

---

# 🔬 Science & Lab

| Category | Icons |
|---|---|
| **Lab Equipment** | 🧪 test tube · 🔬 microscope · ⚗️ alembic · 🧫 petri dish |
| **Biology** | 🧬 dna · 🦠 microbe · 🧫 petri dish |
| **Physics & Math** | ⚛️ atom · 🧲 magnet · 🧮 abacus · ° degree |
| **Measurement** | 📏 ruler · 📐 triangle ruler · ⚖️ balance scale · 🌡️ thermometer · 📡 satellite dish |
| **Optics** | 🔭 telescope · 🔬 microscope · 🔍 magnifying glass · 🔎 magnifying glass right |

---

# 🌌 Space & Cosmic

| Category | Icons |
|---|---|
| **Deep Space** | 🌌 milky way · 🪐 planet · ⭐ star · 🌟 glowing star · 💫 dizzy star · ✨ sparkles |
| **Solar Bodies** | ☀️ sun · 🌕 full moon · 🌑 new moon · 🌒 🌓 🌔 🌖 🌗 🌘 moon phases · 🌙 crescent moon |
| **Visitors & Craft** | ☄️ comet · 🌠 shooting star · 🛸 flying saucer · 👽 alien |
| **Human Spaceflight** | 🚀 rocket · 🛰️ satellite · 🧑‍🚀 astronaut · 📡 satellite dish |

---

# 🧠 Analysis

| Category           | Icons               |
|:-------------------|:--------------------|
| **Reasoning**      | 🧠 brain            |
| **Investigation**  | 🔍 magnifying glass · 🕵️ detective |
| **Metrics / Data** | 📊 bar chart · 📈 chart up · 📉 chart down |
| **Tradeoffs**      | ⚖️ balance scale    |
| **Objectives**     | 🎯 bullseye         |

---

# 🛡️ Security

| Category | Icons |
|---|---|
| **Protection** | 🛡️ shield · 🔐 locked with key · 🔒 locked |
| **Keys / Identity** | 🗝️ old key · 🪪 id card |
| **Policy / Compliance** | 📜 scroll · ✅ check mark · ⚖️ balance scale |
| **Threats** | 🐞 bug · 🧨 firecracker · 🚨 siren |

---

# 🧪 Testing & Quality

| Category | Icons |
|---|---|
| **Testing** | 🧪 test tube · 🔬 microscope · 🧫 petri dish |
| **Verification** | ✅ check mark · ✔️ check · ☑️ ballot box |
| **Bugs** | 🐞 bug |
| **Measurement** | 📏 ruler · 📐 triangle ruler · 📊 bar chart |

---

# 📦 Delivery & Operations

| Category | Icons |
|---|---|
| **Deployment** | 🚀 rocket · 🏁 checkered flag |
| **Artifacts** | 📦 package |
| **Runtime / Infra** | 🖥️ desktop · ☁️ cloud · 🗄️ file cabinet |
| **Telemetry** | 🧾 receipt · 📈 chart up · 📊 bar chart · 🧵 thread |
| **Incidents** | 🚨 siren · 🔥 fire · ⚠️ warning |
| **Maintenance** | 🛠️ hammer wrench · 🔧 wrench · 🧰 toolbox |

---

# 💧🔥🌱 System Dynamics

| Category | Icons |
|---|---|
| **Stability / Cooling** | 💧 droplet · 🌊 wave · 🧊 ice · 🫧 bubbles |
| **Risk / Urgency** | 🔥 fire · ⚡ high voltage · 🚨 siren · 💥 collision |
| **Growth / Evolution** | 🌱 seedling · 🌿 herb · 🌳 tree · ♻️ recycle |
| **Visibility / Cycles** | ☀️ sun · 🌙 crescent moon · ✨ sparkles · 🌥️ sun behind cloud |
| **Variability / Change** | 🌬️ wind face · 💨 dash · 🪁 kite |

---

# 🌪️ Weather

| Category | Icons |
|---|---|
| **Storms** | 🌪️ tornado · ⛈️ thunderstorm · 🌧️ rain cloud · 🌩️ lightning cloud |
| **Rain / Shelter** | ☔ umbrella rain · 🌧️ rain cloud |
| **Sun / Clear** | ☀️ sun · 🌤️ sun small cloud · 🌥️ sun large cloud · 🌦️ sun rain cloud |
| **Temperature** | 🌡️ thermometer · ° degree |
| **Fog / Obscurity** | 🌫️ fog |
| **Cosmic** | ☄️ comet · ✨ sparkles · 🌠 shooting star |

---

# 🌐 External Environment

| Category | Icons |
|---|---|
| **Global Context** | 🌐 globe meridians · 🌍 globe europe · 🌎 globe americas · 🌏 globe asia |
| **Space / External** | 🛰️ satellite · 🪐 planet · 🌌 milky way · 🛸 flying saucer · ✨ sparkles |
| **Constraints** | 🏔️ snow mountain · ⛰️ mountain · 🗻 mount fuji · 🪨 rock |
| **Settlements** | 🏕️ camping · 🏘️ houses · 🏠 house · 🏡 house w/garden · 🏤 european post office |
| **Biomes / Terrain** | 🏞️ national park · 🏜️ desert · 🏝️ island · 🏖️ beach · 🌋 volcano |
| **Exploration** | 🧭 compass · 🗺️ world map · 🔭 telescope |
| **Time / Waiting** | ⏳ hourglass · 🕰 mantel clock |

---

# ➡️ Arrows & Direction

| Category | Icons |
|---|---|
| **Cardinal** | ➡️ right · ⬅️ left · ⬆️ up · ⬇️ down |
| **Diagonal** | ↗️ up-right · ↘️ down-right · ↙️ down-left · ↖️ up-left |
| **Bidirectional** | ↔️ left-right · ↕️ up-down |
| **Curved** | ⤴️ curve up · ⤵️ curve down · ↩️ curve left · ↪️ curve right |
| **Cycle / Repeat** | 🔄 counterclockwise · 🔃 clockwise · 🔁 repeat · 🔂 repeat once |

### Unicode Arrows (for diagrams)

| Style        | Arrows                                                                |
|--------------|-----------------------------------------------------------------------|
| **Single**   | → right · ← left · ↑ up · ↓ down                                      |
| **Diagonal** | ↗ up-right · ↘ down-right · ↙ down-left · ↖ up-left                   |
| **Double**   | ⇒ right · ⇐ left · ⇑ up · ⇓ down                                      |
| **Long**     | ⟶ long right · ⟵ long left · ⟹ long double right · ⟸ long double left |

---

# ⭐ Stars & Decorative

| Category        | Icons                                                                                     |
|-----------------|-------------------------------------------------------------------------------------------|
| **Emoji Stars** | ⭐ star · 🌟 glowing star · ✨ sparkles                                                     |
| **Filled**      | ★ black star · ⭑ small black star · ✦ four-point filled · ✴ eight-point star              |
| **Outlined**    | ☆ white star · ✧ four-point outline · ✰ shadowed star                                     |
| **Ornate**      | ✪ circled star · ⍟ circled star alt · ✫ open star · ✯ pinwheel star · ❂ circled open star |
| **Asterisk**    | ✹ heavy asterisk · ✸ rectilinear star                                                     |
| **Math**        | ⋆ star operator · ≛ star equals · ⍣ star diaeresis                                        |

---

# ☢️ Hazard Symbols

| Category   | Icons                                   |
|------------|-----------------------------------------|
| **Hazard** | ☢️ radioactive · ☣️ biohazard · 👹 ogre  |
| **Alert**  | 🚨 siren · ⚠️ warning · 🚧 construction |

---

# Diagram Grammar

| Icon | Name         | Meaning               |
|:----:|--------------|-----------------------|
|  🧩  | puzzle piece | Service               |
|  🔌  | plug         | API / Port            |
| 🗄️  | file cabinet | Database              |
|  📣  | megaphone    | Event                 |
|  🔗  | link         | Dependency            |
|  🧵  | thread       | Stream                |
|  📊  | bar chart    | Metric                |
|  🔥  | fire         | Architectural hotspot |

---

# Canonical Icons
| Icon  | Name             | Meaning                         | Alternates                              |
|:-----:|------------------|---------------------------------|-----------------------------------------|
|  🤖   | robot            | AI/Autonomous                   | AI/ML (geometry)                        |
|   ✨   | sparkles         | solution/idea/intelligence      |                                         |
|  🧠   | brain            | Semi-Supervised Learning        | Planning (scrum), Neural Network        |
|  🦾   | mechanical arm   | Robotics/Automation             |                                         |
|  🦿   | mechanical leg   | Robotics/Prosthetics            |                                         |
|  🏛   | classical blding | Architecture                    |                                         |
|  🏢   | office           | Architecture/Module             |                                         |
|  🏰   | castle           | Goal/Win/something              |                                         |
|  🧩   | puzzle piece     | Component                       | Self-organizing (scrum)                 |
|  ♻️   | recycle          | Continuous improvement          | Recycle/Reuse                           |
|   ✅   | check mark       | Done/Finished                   |                                         |
|  ⛓️   | chains           | Coupling                        |                                         |
|  🔗   | link             | Cross-cutting dependency        | Linked                                  |
|  🧱   | brick            | Foundation/Object               |                                         |
|  🐞   | bug              | Bug/Flaw                        |                                         |
|  📚   | books            | References                      | Documentation/TOC                       |
|  🗂️  | dividers         | Folder                          |                                         |
|  📜   | scroll           | Table of Contents               |                                         |
|  📊   | bar chart        | Metrics/Chart                   |                                         |
|  📏   | ruler            | Measure                         |                                         |
|  📐   | triangle ruler   | Precision                       | Geometry (geometry), Framework (scrum)  |
|  🧮   | abacus           | Prediction/Calculation          | Counting (geometry)                     |
|  🔎   | magnifying glass | Search                          |                                         |
|  🔭   | telescope        | Examine/Outside                 |                                         |
|  🔬️  | microscope       | Scrutiny                        | Examination (geometry)                  |
|  ⚙️   | gear             | Inner workings                  |                                         |
|  🏗   | crane            | Structure/Build                 |                                         |
|  🧲   | magnet           | Force/Attraction                |                                         |
|  🧰   | toolbox          | Tool, work                      |                                         |
|  🛠️  | hammer wrench    | Tool, work                      |                                         |
|  🔧   | wrench           | Tool, work                      |                                         |
|  🔩   | nut and bolt     | Fastener/Assembly               |                                         |
|  🔨   | hammer           | Build/Forge                     |                                         |
|  💡   | light bulb       | Idea/Illumination               |                                         |
|  📡   | satellite dish   | Signal/Broadcast                |                                         |
|  🚧   | construction     | Under construction              |                                         |
|  🚪   | door             | Door/destination                |                                         |
|  🪜   | ladder           | Climb/elevate                   |                                         |
|  🧵   | thread           | Thread                          |                                         |
|  🤝   | handshake        | Handshake/Agreement             |                                         |
|  🔌   | plug             | Port                            |                                         |
|  🌀   | cyclone          | Pipeline/Process                | Workflow (scrum)                        |
|  🌊   | wave             | Flow                            |                                         |
|  🛰️  | satellite        | External dependency             |                                         |
|  🔥   | fire             | Danger/Risk                     |                                         |
|  💧   | droplet          | Stability                       |                                         |
|  🌱   | seedling         | Growth/Evolution                |                                         |
|   ⏳   | hourglass        | Delay/Waiting                   |                                         |
|  🕰   | mantel clock     | Legacy/Historical               | Time/old                                |
| 🧑‍💻 | technologist     | Developer/Engineer              |                                         |
|  💻   | laptop           | Programming/Computer            |                                         |
|  🧟   | zombie           | Zombie/Stagnation               | Zombie work/Rollover (scrum)            |
|   ⚓   | anchor           | Structural anchor/stability     |                                         |
|  👹   | ogre             | Danger/horror                   |                                         |
|  🖼️  | framed picture   | UI/Presentation                 | Art/visual (geometry)                   |
|  🏦   | bank             | Debt/Cost                       | Pain Zone (geometry), Entropy Tax (scrum) |
| 🤷‍♂️ | shrug            | Pointless/useless               |                                         |
|   ⏲   | timer            | Speed/Timing (alias ⏱️)         |                                         |
| ⛓️‍💥 | broken chain     | Governance Break                | Side Effects/Decoupling (geometry)      |
|  🔢   | input numbers    | Number/Integer                  |                                         |
|  🔤   | input latin      | String/Text                     |                                         |
|  😎   | sunglasses face  | Emojicode/Logic                 |                                         |
|  👽   | alien            | Unknown/External                |                                         |
|  🛸   | flying saucer    | UFO/Unidentified system         |                                         |
|  🌌   | milky way        | Deep space/Vast scope           |                                         |
|  🌠   | shooting star    | Aspiration/Transient            |                                         |
|  🦠   | microbe          | Contagion/Spreading Problem     | Bug (biological)                        |
|  ⚗️   | alembic          | Distillation/Refinement         |                                         |
|  ⚛️   | atom             | Atomic/Fundamental unit         |                                         |
|  🧫   | petri dish       | Experiment/Culture              |                                         |
| 🧑‍🔬 | scientist        | Research/Science                |                                         |
| 🧑‍🚀 | astronaut        | Exploration/Space               |                                         |
|  👷   | construction wkr | Builder/Implementation          |                                         |
|  🕵️  | detective        | Investigation/Debugging         |                                         |
|  🧬   | dna              | Genetics/Training               | Complexity (scrum), Main Sequence (geometry) |
|  🔃   | clockwise arrows | Closed Loop                     |                                         |
|  👥   | silhouettes      | Team/Governance                 | Team/Group (geometry)                   |
|  📑   | bookmark tabs    | Data/Outcome                    |                                         |
|  🔍   | magnifying glass | Investigation                   |                                         |
|  🧭   | compass          | Context/Direction               |                                         |
|  📖   | open book        | References                      | Table of Contents (scrum)               |
|  💨   | dash             | Afferent/Inward                 | Motion (scrum)                          |
|  🌬   | wind face        | Efferent/Outward                | Agility (scrum)                         |
|  📈   | chart up         | Trending/Curves                 |                                         |
|  📉   | chart down       | Declining/Lowering              |                                         |
|  🌐   | globe meridians  | Abstractness/Global             |                                         |
|  🪨   | rock             | Danger Zone                     |                                         |
|  📍   | pushpin          | Target/Focus                    |                                         |
|  🗺️  | world map        | Landscape/Applicability         | Map/Territory (scrum)                   |
|  🪐   | planet           | Universe/Vast scope             |                                         |
|  📋   | clipboard        | Work Items/Meetings             |                                         |
|  📦   | package          | Release/Delivery                |                                         |
|  🧊   | ice              | Frozen/Batched                  |                                         |
|  🌫️  | fog              | Illusion/Obscurity              |                                         |
|  ⚖️   | balance scale    | Fairness/Warning                |                                         |
|  🗿   | moai             | Legacy/Monument                 |                                         |
|  ⛏️   | pick             | Archaeology/Digging             |                                         |
|  🛤️  | railway          | Track/Rails/Path                |                                         |
|  💦   | sweat droplets   | Afferent Coupling               |                                         |
|  📃   | page with curl   | Introduction                    |                                         |
|  🚰   | potable water    | Bottleneck                      |                                         |


## Final
**_Quick-reference: all icons by category_**

| Category                    | Icons                                                                                                                                                                                                                                                                                                                                                            |
|:----------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **AI & Thinking**           | 🤖 robot:`AI` · 🧠 brain:`ML/thinking` · ✨ sparkles:`idea/AI` · 🎓 graduation cap:`learning` · 🦾 mechanical arm:`automation` · 🦿 mechanical leg:`robotics`                                                                                                                                                                                                   |
| **Architecture**            | 🏛️ classical building:`architecture` · 🏢 office:`module` · 🏦 bank:`institution` · 🏰 castle:`goal/stronghold` · 🏚️ derelict house:`legacy` · 🧱 brick:`object` · 🗿 moai:`monument` · 🪨 rock:`bedrock`                                                                                                                                                      |
| **Buildings & Settlements** | 🏠 house · 🏡 house w/garden · 🏘️ houses · 🏤 european post office · 🏕️ camping:`temporary` · 🏞️ national park · 🏜️ desert · 🏝️ island · 🏖️ beach                                                                                                                                                                                                         |
| **Construction & Tools**    | 🏗️ crane:`build` · 🧰 toolbox:`tools` · 🛠️ hammer wrench:`tool` · 🔧 wrench · ⚙️ gear:`inner workings` · 🧲 magnet:`attraction` · 🚧 construction:`WIP` · 🚪 door:`entry/exit` · 🪜 ladder:`elevate` · 🔩 nut and bolt · 🔨 hammer · ⛏️ pick · 🪛 screwdriver · 🪚 saw                                                                                       |
| **Electrical & Hardware**   | 🔋 battery · 🪫 low battery · 💡 light bulb:`idea` · 🔦 flashlight · ⚡ high voltage:`urgency` · 📡 satellite dish:`signal`                                                                                                                                                                                                                                     |
| **People & Roles**          | 👤 silhouette · 👥 silhouettes:`team` · 🫂 hugs · 🕴️ levitating · 🧍 standing · 🚶 walking · 👫 couple · 🤝 handshake:`agreement`                                                                                                                                                                                                                              |
| **People at Work**          | 🧑‍💻 technologist:`developer` · 👷 construction worker:`builder` · 🧑‍🔬 scientist:`research` · 🧑‍🏭 factory worker · 🧑‍💼 office worker · 🧑‍🎓 student · 🧑‍🔧 mechanic · 🧑‍✈️ pilot · 🧑‍🚀 astronaut · 🧑‍⚕️ health worker · 🧑‍🏫 teacher · 🕵️ detective:`investigation`                                                                              |
| **Navigation & Routing**    | 🗺️ world map:`landscape` · 🌐 globe:`global` · 🧭 compass:`direction` · 📍 pushpin:`location` · 🛣️ motorway:`path` · 🛤️ railway:`track` · 🚦 traffic light:`gate` · 🌉 bridge:`crossing` · 🔎 magnifying glass:`search` · 🔭 telescope:`explore outside`                                                                                                      |
| **Documentation**           | 📚 books:`docs/TOC` · 📖 open book:`reference` · 📙 orange book · 📗 green book · 📘 blue book · 🗒️ notepad:`notes` · 📝 memo:`draft` · 📄 page:`document` · 📑 bookmark tabs:`sections` · 📜 scroll:`contract/TOC` · 📃 page with curl:`terms`                                                                                                                 |
| **Storage & Organization**  | 🗃️ card file box:`archive` · 🗄️ file cabinet:`database` · 🗂️ dividers:`folder` · 📋 clipboard:`work items` · 🏷️ label:`tag` · 🔖 bookmark:`version` · 📌 pushpin:`pinned`                                                                                                                                                                                    |
| **Contracts & Validation**  | 🧾 receipt:`receipt/log` · 🔌 plug:`API/port` · 🧬 dna:`schema/signature` · 📐 triangle ruler:`blueprint` · 📏 ruler:`measure` · ✅ check mark:`done` · ☑️ ballot box:`validated` · ✔️ check:`verified` · 🧪 test tube:`test`                                                                                                                                     |
| **Components & Connectors** | 🧩 puzzle piece:`component` · 📦 package:`artifact` · ➕ plus:`compose` · 🔗 link:`dependency` · ⛓️ chains:`coupling` · ⛓️‍💥 broken chain:`decoupling` · 🧷 safety pin:`adapter` · 👥 people:`team` · 🤝 handshake:`agreement` · ♻️ recycle:`pattern/reuse`                                                                                                     |
| **Flow & Orchestration**    | 🌀 cyclone:`workflow` · 🌊 wave:`flow` · 🔄 cycle:`iteration` · 🎛️ control knobs:`orchestration` · 🧵 thread:`stream` · 📣 megaphone:`event` · 📨 envelope:`message` · 🛰️ satellite:`external dep`                                                                                                                                                             |
| **Status & Time**           | 🟢 green:`healthy` · 🟡 yellow:`caution` · 🔴 red:`critical` · ☯ yin yang:`balance` · ⏱️ stopwatch:`timing` · ⏳ hourglass:`waiting` · 🕒 clock:`schedule` · 🕰 mantel clock:`legacy/time`                                                                                                                                                                        |
| **Science & Lab**           | 🧪 test tube:`test` · 🔬 microscope:`examine` · ⚗️ alembic:`distill` · 🧫 petri dish:`experiment` · 🧬 dna:`genetics` · 🦠 microbe:`bio` · ⚛️ atom:`fundamental` · ° degree                                                                                                                                                                                    |
| **Space & Cosmic**          | 🌌 milky way:`deep space` · 🪐 planet:`external` · 🛸 flying saucer:`UFO` · 👽 alien:`unknown` · 🌠 shooting star:`transient` · 🌕 full moon · 🌑 new moon · 🌒 🌓 🌔 🌖 🌗 🌘 moon phases · 💫 dizzy star · 🚀 rocket:`deploy` · 🛰️ satellite · 🧑‍🚀 astronaut                                                                                              |
| **Metrics & Analysis**      | 📊 bar chart:`metrics` · 📈 chart up:`trending` · 📉 chart down · 🧮 abacus:`calculation` · ⚖️ balance scale:`tradeoff` · 🎯 bullseye:`objective` · 🔍 magnifying glass:`investigate` · 🔬 microscope:`examine inside` · 🕵️ detective                                                                                                                           |
| **Security & Threats**      | 🛡️ shield:`protection` · 🔐 locked with key:`secure` · 🔒 locked:`restricted` · 🗝️ old key:`secret` · 🪪 id card:`identity` · 🐞 bug:`defect` · 🧨 firecracker:`explosive risk` · 🚨 siren:`alert` · ⚠️ warning:`caution`                                                                                                                                     |
| **Delivery & Runtime**      | 🚀 rocket:`deploy` · 🏁 checkered flag:`milestone` · 🖥️ desktop:`server` · ☁️ cloud:`cloud infra` · 🧑‍💻 technologist:`developer` · 💻 laptop:`programming` · 🚰 potable water:`bottleneck`                                                                                                                                                                    |
| **Backlog & Dead Work**     | 🧟 zombie:`zombie work` · 🪦 headstone:`dead work` · 🧊 ice:`frozen work` · 🤷‍♂️ shrug:`pointless`                                                                                                                                                                                                                                                              |
| **Growth & Dynamics**       | 🌱 seedling:`evolution` · 🌿 herb:`growing` · 🌳 tree:`mature` · 💧 droplet:`stability` · 💦 sweat droplets · 🫧 bubbles:`froth` · 🔥 fire:`risk/hotspot` · ⚡ high voltage:`urgency` · 💥 collision:`breaking change` · 🌬️ wind face:`variability` · 💨 dash:`volatility` · 🪁 kite:`agility`                                                                    |
| **Visibility & Cycles**     | ☀️ sun:`visible/clear` · 🌙 crescent moon:`hidden/dark` · 🌥️ sun behind cloud:`partial` · 🌤️ sun small cloud:`mostly clear` · 🌦️ sun rain cloud:`mixed`                                                                                                                                                                                                       |
| **Weather & Storms**        | 🌪️ tornado:`chaos` · ⛈️ thunderstorm:`crisis` · 🌧️ rain cloud:`downturn` · 🌩️ lightning cloud:`disruption` · ☔ umbrella rain:`protection` · 🌡️ thermometer:`temperature` · 🌫️ fog:`obscurity` · ☄️ comet:`impact`                                                                                                                                           |
| **External Environment**    | 🌍 globe europe · 🌎 globe americas · 🌏 globe asia · 🪐 planet:`external` · 🏔️ snow mountain:`constraint` · ⛰️ mountain:`obstacle` · 🗻 mount fuji:`landmark` · 🌋 volcano:`eruption` · 🏕️ camping:`temporary` · 🏘️ houses · 🏞️ national park · 🏜️ desert:`barren` · 🏝️ island:`isolated` · 🏖️ beach:`edge`                                               |
| **Arrows & Direction**      | ➡️ right · ⬅️ left · ⬆️ up · ⬇️ down · ↗️ ↘️ ↙️ ↖️ diagonals · ↔️ ↕️ bidirectional · ⤴️ ⤵️ ↩️ ↪️ curves · 🔃 clockwise · 🔁 repeat · 🔂 repeat once                                                                                                                                                                                                           |
| **Stars & Symbols**         | ⭐ star · 🌟 glowing star · ★ black star · ☆ white star · ✦ ✧ four-point · ✴ eight-point · ✪ ⍟ ✫ ✯ ✰ ❂ ornate · ✹ ✸ asterisk · ⋆ star operator · ☢️ radioactive · ☣️ biohazard                                                                                                                                                                                |
